...
Pour lire le flux vidéo, nous utilisons un extracteur qui permet de parser un fichier vidéo. 
Nous pouvons ensuite demander à l'extracteur le type de la vidéo contenue dans le fichier, puis d'écrire une partie partie du flux vidéo dans un buffer.

\bigbreak
Pour décoder ce flux vidéo, nous utilisons l'API MediaCodec qui permet d'accéder au support multimédia bas-niveau fournit par Android. Ce MediaCodec permet, entre autres, de décoder un flux vidéo.
Pour fonctionner, nous configurons un mediaCodec avec les paramètres de la vidéo (trouvés via l'extracteur) ainsi que la surface dans laquelle la vidéo décodée doit être envoyée (cette partie sera expliquée dans la sous-section suivante).
Grâce à notre thread, nous allons régulièrement demander à l'extracteur les données suivantes du flux vidéo, et nous transferons ces données au mediaCodec.
Le mediaCodec décode alors cette nouvelle partie du flux vidéo et envoye les images décodées à la surface.

\subsubsection{Récepteur réseau}
La deuxième implémentation du module de réception vidéo est similaire à la précédante, hormis que l'on récupère le flux vidéo sur un équipement distant.
Ce module remplace donc l'extrateur par deux sockets réseaux.

\bigbreak
Tout d'abord, le récepteur se connecte à l'émetteur via une socket TCP pour initier le canal de contrôle.
C'est sur ce canal que les informations conncernant la ZOI sont échangées entre l'émetteur et le récepteur.

\bigbreak
Puis, l'application écoute sur un port UDP pour récupérer le flux vidéo envoyé par l'émetteur.
Plus précisement, les datagrammes UDP contiennent des paquets MPEG-TS.
Ces paquets MPEG-TS encapsulent des fragments de paquets PES (\textit{Packetized Elementary Stream}) qui correspondent aux données du flux vidéo compressé.
Une fois un paquet PES reconstitué (à l'aide de plusieurs paquets MPEG-TS), il est transféré au mediaCodec.
Le MediaCodec décode alors la partie du flux vidéo qui est présent dans le paquet et envoye les images décodées à la surface.
...
\bigbreak
Afin de produire un effet immersif, nous effectuons un traitement OpenGL ES dans la fonction de \textit{callback}.
...
\bigbreak
Lorsqu'un déplacement de la ZOI est commandé par le récepteur, les nouvelles coordonnées désirées de la ZOI sont alors envoyées à l'émetteur via le canal de contrôle TCP.
L'émetteur commence alors à générer les futures images en prenant en compte cette nouvelle ZOI et renvoie aussitôt sur le canal TCP le numéro de la frame à partir de laquelle cette nouvelle ZOI est appliquée.
...
Heureusement, chaque paquet PES transmis est encodé avec le PTS, \textit{Presentation timestamp}, qui indique à quel moment afficher l'image. Cet indicateur est ajouté à la compression et est donc entièrement synchronisé sur l'émetteur.
Comme nous encodons avec un taux d'image constant, nous pouvons alors en déduire le numéro de l'image.
Sur l'application Android, MediaCodec nous fournit cette information à la mise à jour de la surface.
En revanche, sur le réception Linux qui reçoit simplement les octets correspondant aux images RAW, il n'y a pas de métadonnées transmises.
Nous utilisons alors le module \texttt{showinfo} de FFmpeg qui écrit sur la sortie standard les informations de chaque image. La sortie est redirigée sur une entrée de notre récepteur qui connaît donc le numéro de frame en cours.
